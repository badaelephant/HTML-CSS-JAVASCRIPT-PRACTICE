const data = [
  ...
  { idx: 6, title: "joker", src: "/images/joker.jpg", rate: 8.4 },
...
export function Container() {
  ...
  const lastpage =
    movies.length === 0 ? 0 : Math.floor((movies.length - 1) / size);
    ...
    <>
      <div>
        <form className="flex space-x-5" onSubmit={onSubmitSearch}>
          ...
          <button
            type="submit"
            className="py-2 px-6 bg-green-400 border-4 rounded-full shadow text-white font-bold hover:bg-green-800"
          >
            Search
          </button>
        </form>
      ...
      >
        {movies.map((movie, i) =>
          movie.idx < (page + 1) * size && movie.idx >= page * size ? (
            <Card movie={movie} key={i} idx={i} />
          ) : null
        )}
      </div>
      <div className="flex space-x-4 mt-4">
        {page === 0 ? (
          <button className="px-3 py-3 bg-red-500 rounded-xl text-white border-2 border-gray-200 opacity-30">
            PREV
          </button>
        ) : (
          <button
            className="px-3 py-3 bg-red-500 rounded-xl text-white border-2 border-gray-200"
            onClick={onClickPrev}
          >
            PREV
          </button>
